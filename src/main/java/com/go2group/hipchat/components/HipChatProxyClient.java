...
public class HipChatProxyClient implements DisposableBean {
	...
	public HipChatProxyClient(RequestFactory<Request<?, Response>> requestFactory,
	 ...
	 * @return JSON String
	 */
	public JSONString getUser(final String serverUrl, final String userId, final String authToken) {
		final String url = serverUrl + "/v1/users/show?user_id=" + userId;
		return jsonGet(url, authToken);
	}

	private String getIconUrl(String baseUrl) {
		return "<img src=\"" + baseUrl + "/images/icons/blogentry_16.gif" + "\" width=16 height=16 />&nbsp;";
	}
	 ...
	 */
	public JSONString getRooms(String serverUrl, String authToken) throws InvalidAuthTokenException {
	 ...
	 */
	public void notifyRoom(final String room, final String message, final String color, final String authToken, final String serverUrl) {
		...
		Runnable postRequest = new Runnable() {
			...
			public void run() {
				...
				request.addRequestParameters("room_id", room, "from", "Bamboo", "message", message, "color",
						color == null ? "yellow" : color, "format", "json");
				try {
	 ...
	 * 
	 * @param url
	 * @return String containing JSON if successful, empty String otherwise.
	 *         Never returns null. This will always return an empty string if
	 *         the Hip-Chat authentication token is not set.
	 */
	private JSONString jsonGet(String url, String authToken) {
		if (StringUtils.isEmpty(authToken) || StringUtils.isEmpty(url)) {
			return new JSONString("");
		}
		final String urlWithAuthToken = url.contains("?") ? url + "&auth_token=" + authToken : url + "?auth_token="
				+ authToken;
